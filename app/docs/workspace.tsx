...
export default function Workspace({ initialId }: { initialId: string | null }) {
  ...
  // ── Persist URL in sync with selectedId ───────────────────────────────────
  ...
  useEffect(() => {
    getAllDocs().then((loaded) => {
      setDocs(loaded)
      if (initialId && !loaded.find((d) => d.id === initialId)) {
        setNotFound(true)
        setSelectedId(null)
      }
    })
  ...
  // Auto-focus title when a blank doc is opened in edit mode
  useEffect(() => {
  ...
  function DocRow({ doc }: { doc: Doc }) {
    const isPending = confirmDeleteId === doc.id
    return (
      <div className={`group relative flex items-center rounded-md mb-0.5 transition-colors ${
        ...
        <button onClick={() => handleSelect(doc.id)} className="flex-1 text-left px-3 py-2 min-w-0">
          ...
          <p className="text-xs text-gray-400 dark:text-gray-500 hc:text-gray-300 mt-0.5">{formatUpdated(doc.updatedAt)}</p>
          {(doc.tags ?? []).length > 0 && (
  ...
  function SectionLabel({ label }: { label: string }) {
    return (
      <p className="px-3 pb-1 text-xs font-medium text-gray-400 dark:text-gray-500 hc:text-gray-400 uppercase tracking-wide">
        {label}
      </p>
    )
  }
          ...
          {notFound ? (
            <div className="flex-1 flex items-center justify-center p-6">
              <div className="text-center max-w-xs">
                ...
                <p className="text-sm text-gray-400 dark:text-gray-500 hc:text-gray-300 mt-1 leading-relaxed">This document may have been deleted or the link is incorrect.</p>
                <Link href="/docs" onClick={() => setNotFound(false)} className="inline-block mt-4 px-4 py-2 bg-gray-900 dark:bg-white hc:bg-white text-white dark:text-gray-900 hc:text-black text-sm font-medium rounded-md hover:bg-gray-700 dark:hover:bg-gray-100 transition-colors">
                  Back to workspace
                </Link>
              </div>
            </div>
              ...
              {showHistory && (
                <div className="bg-gray-50 dark:bg-gray-800/60 hc:bg-gray-950 border-b border-gray-200 dark:border-gray-700 hc:border-white px-4 md:px-6 py-3">
                  ...
                  {!selectedDoc.history?.length ? (
                    <p className="text-xs text-gray-400 dark:text-gray-500 hc:text-gray-400">No saved versions yet. Click <strong className="font-medium">Save</strong> in the toolbar to capture one.</p>
                  ) : (
          ...
          <div className="relative bg-white dark:bg-gray-800 hc:bg-black border border-gray-200 dark:border-gray-600 hc:border-white rounded-xl shadow-xl w-full max-w-sm p-6">
            ...
            <div className="border-t border-gray-100 dark:border-gray-700 hc:border-gray-800 pt-5">
              ...
              {importStep.step === "review" && (
                <div>
                  <p className="text-xs text-gray-700 dark:text-gray-200 hc:text-white mb-3">
                    Found <strong>{importStep.parsed.length}</strong> document{importStep.parsed.length !== 1 ? "s" : ""}. How would you like to import?
                  </p>
              ...
              {importStep.step === "confirm" && (
                <div>
                  <p className="text-xs text-gray-700 dark:text-gray-200 hc:text-white mb-1"><strong>Are you sure?</strong></p>
